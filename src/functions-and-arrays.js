...
function sum(mixedArr) {
  ...
  for (i = 0; i < mixedArr.length; i++) {
    if (typeof mixedArr[i] === "string") {
      ...
      typeof mixedArr[i] === "number" ||
      typeof mixedArr[i] === "boolean"
    ) {
      total += mixedArr[i];
    } else {
...
function averageNumbers(numbersAvg) {
  ...
  const media = result / numbersAvg.length;
  return media;
}
...
function averageWordLength(wordsArr) {
  ...
  let resultStr = 0;
  for (string of wordsArr) {
    resultStr += string.length;
  }
  const mediaStr = resultStr / wordsArr.length;
  return mediaStr;
}
...
function avg(mixedArr) {
  ...
  for (element of mixedArr) {
    if (typeof element === "string") {
      resultMixed += element.length;
    } else if (typeof element === "number" || typeof element === "boolean") {
      resultMixed += element;
    }
  }
  return resultMixed / mixedArr.length;
}
